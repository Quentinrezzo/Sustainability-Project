...
The dataset is sourced from Datastream and covers 2,545 firms with available carbon data. It spans 1999 to 2025 for stock prices and market capitalisations, and 2002 to 2024 for carbon emissions and revenue. Carbon data coverage is limited, especially before 2010, which motivates starting the allocation exercise at the end of 2013 to ensure a solid 12-year test period.
...
Among the MV portfolios, the Ledoit-Wolf variant achieves the lowest volatility (11.\%): shrinkage finds a trade-off between the unbiased but noisy sample covariance and a structured target with low estimation error, producing more diversified and stable weights. However, the smaller FA investment universe limits diversification and excludes some return-enhancing exposures. The sample covariance variant (SLSQP) sits at the other extreme, being unbiased but subject to estimation error when  $N$ is relative large to $\tau$ as explained in Section 2.4.4
...
\subsubsection{Diversification Analysis}

The VW-FA portfolio is naturally dominated by the largest firms by market capitalisation, with the top position reaching approximately 6\% by 2024. The MV portfolio has a different composition, with higher individual weights reaching up to 8\%. This is expected: the minimum-variance optimizer concentrates on firms that exhibit low volatility and favorable correlation properties, assigning them disproportionately large weights compared to a market-cap benchmark that spreads allocation more broadly across the universe.

This degree of diversification indicates that no explicit cap on individual weights was needed in the optimization. Some approaches impose weight limits to avoid excessive concentration, in our case the long-only constraint combined with the Ledoit-Wolf covariance estimator naturally led to more balanced allocations and prevented the portfolio from concentrating in a small number of stocks.
...
\subsubsection{Firms driving up WACI and CF}

Figure~\ref {fig:top10_ci} and Figure~\ref {fig:top10_ci_weights} (Appendices) display the top 10 most carbon-intensive firms in the investment set each year and the corresponding weights these firms carry in the VW-FA portfolio. A small group of firms dominates the carbon intensity ranking throughout the period, with the leading firm reaching values above 40,000~tCO$_2$e/M\$rev in 2022. While the composition of the top 10 evolves, several names reappear consistently across most years, indicating persistent concentration of carbon intensity among a limited number of firms.

Despite their extreme carbon intensity, these firms receive negligible allocations in the VW-FA portfolio. Individual weights rarely exceed 0.20\% and most remain below 0.05\%, reflecting their small market capitalisation relative to the rest of the investment universe. This asymmetry between carbon intensity and portfolio weight suggests that imposing carbon constraints on the portfolio should come at a low financial cost, since the firms that contribute most to carbon metrics can be reduced or excluded without materially altering the portfolio's risk-return profile which will be studied in Section 4.2.3.
...
Figure~\ref{fig:mv_weights_top10_ci} and figure~\ref{fig:mvcf_weights_top10_ci} (Appendices) compare the weights assigned to the top 10 most carbon-intensive firms in the unconstrained MV and CF-constrained MV portfolios. In the unconstrained portfolio, most of these firms already receive zero or near-zero weight, with occasional exceptions where one firm reaches up to 4\% in earlier years. The CF-constrained portfolio shows a similar pattern but further reduces these residual exposures: the few carbon-intensive firms receiving non-negligible weight in the unconstrained solution see their allocation decrease or vanish entirely. From 2019 onward, both portfolios assign essentially zero weight to the top 10 most carbon-intensive firms, confirming that the minimum-variance optimiser naturally avoids these firms for risk reasons and that the 50\% CF constraint requires only marginal adjustments.

Overall, these results indicate that halving the carbon footprint of the minimum-variance portfolio comes at no cost in our investment universe: returns and volatility both decrease marginally, leaving the Sharpe ratio unchanged.
...
\subsection{Comparison of Net Zero and Tracking Error Portfolios}

\begin{table}[H]
\centering
\captionsetup{justification=centering}
\caption{Annualised performance statistics --- VW-FA, VW-FA Drift, TE and NZ.}
\label{tab:stats_nz_all}
\begin{tabular}{lcccc}
\toprule
& \textbf{VW-FA} & \textbf{VW-FA Drift} & \textbf{TE (LW)} & \textbf{NZ (LW)}\\[4pt]
\midrule
Annualised Average Return       & 11.66\% & 11.35\% & 11.28\% & 11.33\% \\[4pt]
Annualised Volatility           & 14.25\% & 14.36\% & 14.36\% & 14.36\% \\[4pt]
Annualised Cumulative Return    & 11.18\% & 10.82\% & 10.75\% & 10.81\% \\[4pt]
Sharpe Ratio                    & 0.70    & 0.67    & 0.66    & 0.67  \\[4pt]
Minimum Monthly Return          & $-$13.08\% & $-$13.80\% & $-$13.78\% & $-13.78$\%  \\[4pt]
Maximum Monthly Return          & 12.86\% & 13.38\% & 13.43\% & 13.39\%  \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:stats_nz_all} allows a direct comparison of the CF-constrained tracking-error and net-zero portfolios with the benchmarks. The TE portfolio delivers a return of 11.28\% with similar variance and a Sharpe ratio of 0.66, very close to the VW-FA Drift portfolio (0.67). This shows that adding a 50\% carbon footprint reduction during portfolio construction alters its performance only marginally. The NZ portfolio performs remarkably close to the VW-FA with drift portfolio despite the carbon constraint. With a return of 11.33\% and a Sharpe ratio of 0.67, the NZ portfolio is nearly indistinguishable from the benchmark. It also performs slightly better than the TE portfolio which might seem suprising at first: the net-zero constraint imposes a compounding constraint that should be more binding than a static 50\% target over time. In our case though the net-zero portfolio is far less binding because the investment universe itself decarbonises over the sample period, as shown in Section~4.1. 
...
Figure~\ref{fig:cumulative_returns_nz} displays the cumulative returns of all portfolios over the full sample period. The TE, NZ, and VW-FA Drift portfolios follow nearly identical trajectories, ending between 3.0 and 3.2 by end-2025. The VW-FA portfolio finishes slightly higher at approximately 3.5, but this difference reflects its monthly rebalancing from market capitalisations rather than a genuine allocation advantage. Since the TE, NZ, and MV portfolios all use annual buy-and-hold weight drift, VW-FA Drift is the appropriate benchmark for comparison. On that basis, the carbon-constrained tracking-error portfolios replicate the benchmark almost exactly. The MV and MV-CF portfolios remain grouped together around 2.2, consistent with their lower-return, lower-risk profile documented in Section~4.2.
...
Figure~\ref{fig:cf_te_cf_nz} shows that the carbon footprint of both the TE and NZ portfolios falls over time. The TE portfolio stays consistently below the NZ portfolio, suggesting that its carbon constraint is more restrictive, while the NZ portfolio follows a path with a carbon ceiling that declines exponentially each year.

Figure~\ref{fig:top10_te} and figure~\ref{fig:top10_nz} (Appendices) are useful for comparing the TE and NZ portfolios because they share the same optimization objective and differ only in the carbon constraint imposed. The figures indicate that the TE portfolio is more aggressive in reducing exposure to the most carbon-intensive firms, with weights that are often zero or close to zero, while the NZ portfolio retains slightly higher allocations to some of these stocks. This pattern is consistent with the fact that the 50\% carbon reduction constraint is more binding than the compounding 10\% annual reduction path, thereby leaving the NZ portfolio with greater flexibility.

This result is in line with the framework proposed by Bolton, Kacperczyk, and Samama (2021), who show that minimising the tracking error under a net-zero carbon budget constraint produces portfolios that remain closely aligned with the benchmark, with the tracking error increasing only gradually as the carbon ceiling tightens over time.
...
These findings are consistent with the academic literature studied in this course. Jondeau, Mojon, and Pereira da Silva (2021) show that applying a 10\% annual carbon reduction to a global equity benchmark over 11 years preserves the Sharpe ratio almost entirely, with negligible tracking error. Similarly, Bolton, Kacperczyk, and Samama (2021) demonstrate that minimising the tracking error under a net-zero carbon budget constraint produces portfolios closely aligned with the benchmark, confirming that carbon constraints can be imposed without significant financial sacrifice when the investment universe is sufficiently broad.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{WACI-based constraints.} The current carbon constraints target CF only. Since WACI normalizes by revenues rather than market capitalisation, it captures a different dimension of carbon exposure. Adding a WACI constraint to CF could prevent the optimizer from focusing on firms that appear clean in CF terms but remain carbon-intensive relative to revenues.
...
This project implements and compares carbon-aware portfolio strategies within a long-only equity framework over 2014--2025. We evaluate three covariance estimators for the minimum-variance portfolio (sample covariance, single-factor model, Ledoit-Wolf shrinkage), illustrating the bias-variance trade-off in covariance estimation. We then construct carbon-constrained portfolios targeting both an active investor (minimum-variance with 50\% CF reduction) and a passive investor (tracking-error minimization with 50\% CF reduction and net-zero trajectory).

The main result is that carbon constraints can be imposed at low financial cost across all strategies. The Sharpe ratios of the constrained portfolios remain very close to those of their unconstrained counterparts, and the tracking-error portfolios maintain benchmark-like performance while achieving substantial carbon reductions. The net-zero strategy, despite its compounding constraint, does not significantly underperform the static 50\% reduction over our sample period.

These favorable results should be interpreted with caution. The sample period coincides with a broad decarbonization trend in the investment universe, which eases carbon constraints. Whether such low costs persist will depend on the pace of firm-level emission reductions. As carbon budgets tighten further and the easiest substitutions are exhausted, investors may face harder trade-offs between decarbonization targets and portfolio performance.

\section{Appendices}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Top10_CF.png}
    \captionsetup{font=footnotesize, justification=centering}
    \caption{Top 10 most carbon-intensive firms in the investment set, 2013--2024.}
    \label{fig:top10_ci}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Top10_WACI.png}
    \captionsetup{font=footnotesize, justification=centering}
    \caption{Weights of the VW-FA portfolio in the top 10 most carbon-intensive firms, 2013--2024.}
    \label{fig:top10_ci_weights}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Top10_MVP_CI.png}
    \captionsetup{font=footnotesize, justification=centering}
    \caption{Minimum-Variance Portfolio (Ledoit-Wolf) weights in top 10 most carbon-intensive firms, 2013--2024.}
    \label{fig:mv_weights_top10_ci}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Top10_MVP_CF.png}
    \captionsetup{font=footnotesize, justification=centering}
    \caption{MV CF-Constrained Portfolio (Ledoit-Wolf) weights in top 10 most carbon-intensive firms, 2013--2024.}
    \label{fig:mvcf_weights_top10_ci}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Top10_TE.png}
    \captionsetup{font=footnotesize, justification=centering}
    \caption{Tracking Error Portfolio (Ledoit-Wolf) weights in top 10 most carbon-intensive firms, 2013--2024.}
    \label{fig:top10_te}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/Top10_NZ.png}
    \captionsetup{font=footnotesize, justification=centering}
    \caption{Net-Zero Portfolio (Ledoit-Wolf) weights in top 10 most carbon-intensive firms, 2013--2024.}
    \label{fig:top10_nz}
\end{figure}
